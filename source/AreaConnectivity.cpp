...
std::unique_ptr<AreaConnectivity::AreaConnecMap> AreaConnectivity::compute_area_connectivity_strength(DistributedGraph& graph, const unsigned int result_rank) {
	...
	std::function<std::unique_ptr<std::vector<std::tuple<std::uint64_t, std::uint64_t, AreaConnectivityInfo>>>(const DistributedGraph&, std::uint64_t)>
	    transfer_connection_sources = [&](const DistributedGraph& dg, std::uint64_t node_local_ind) {
		    ...
		    auto connection_sources = std::make_unique<std::vector<std::tuple<std::uint64_t, std::uint64_t, AreaConnectivityInfo>>>(out_edges.size());

		    std::transform(out_edges.cbegin(), out_edges.cend(), connection_sources->begin(), [&](const OutEdge& oEdge) {
			    AreaConnectivityInfo one_connection = {my_rank, node_areaID, oEdge.target_rank, -1, oEdge.weight};
			    return std::tuple<std::uint64_t, std::uint64_t, AreaConnectivityInfo>{oEdge.target_rank, oEdge.target_id, one_connection};
		    });

		    return std::move(connection_sources);
	    };

	std::function<AreaConnectivityInfo(const DistributedGraph&, std::uint64_t, AreaConnectivityInfo)> fill_connection_target_areaID =
	...
	std::unique_ptr<CommunicationPatterns::NodeToNodeQuestionStructure<AreaConnectivityInfo, AreaConnectivityInfo>> area_connections =
	    CommunicationPatterns::node_to_node_question<AreaConnectivityInfo, AreaConnectivityInfo>(
		graph, MPIWrapper::MPI_AreaConnectivityInfo, transfer_connection_sources, MPIWrapper::MPI_AreaConnectivityInfo, fill_connection_target_areaID);

	AreaIDConnecMap my_rank_connecID_map;
	
	for (std::uint64_t node_ID = 0; node_ID < number_local_nodes; node_ID++) {
		std::unique_ptr<std::vector<AreaConnectivityInfo>> node_connections = area_connections->getAnswersOfQuestionerNode(node_ID);
	...
	std::function<std::unique_ptr<std::vector<std::pair<connecID_map_data, int>>>(const DistributedGraph&)> extract_connecID_map =
	    [&](const DistributedGraph& dg) {
		    ...
		    for (auto map_entry = my_rank_connecID_map.cbegin(); map_entry != my_rank_connecID_map.cend(); map_entry++) {
			    ...
			    auto connecID_data_entry = std::tie(map_entry->first.first.first, map_entry->first.first.second, map_entry->first.second.first,
								map_entry->first.second.second, map_entry->second);
	...
	std::function<std::unique_ptr<std::vector<std::pair<std::string, int>>>(const DistributedGraph&)> get_names = [](const DistributedGraph& dg) {
	...
	std::unique_ptr<std::vector<std::vector<std::string>>> area_names_list_of_ranks = CommunicationPatterns::gather_Data_to_one_Rank<std::string, char>(
	    graph, get_names, [](std::string area_name) { return std::vector<char>(area_name.cbegin(), area_name.cend()); },
	    [](std::vector<char> area_name_vec) { return std::string(area_name_vec.data(), area_name_vec.size()); }, MPI_CHAR, result_rank);
...
std::unique_ptr<AreaConnectivity::AreaConnecMap> AreaConnectivity::area_connectivity_strength_sequential_helge(const DistributedGraph& graph,
													       unsigned int result_rank) {
	const int my_rank = MPIWrapper::get_my_rank();
	const int number_of_ranks = MPIWrapper::get_number_ranks();
	const std::uint64_t number_local_nodes = graph.get_number_local_nodes();

	std::function<std::unique_ptr<std::vector<std::pair<std::uint64_t, int>>>(const DistributedGraph& dg)> get_number_nodes =
	    [&](const DistributedGraph& dg) {
		...
		for (int rank = 0; rank < number_of_ranks; rank++) {
			for (std::uint64_t node_local_ind = 0; node_local_ind < (*ranks_to_number_local_nodes)[rank][0]; node_local_ind++) {
				...
				for (const OutEdge& out_edge : out_edges) {
					...
					std::int64_t target_node_areaID = graph.get_node_area_localID(out_edge.target_rank, out_edge.target_id);

					std::string source_area = (*area_names_list_of_ranks)[rank][source_node_areaID];
					std::string target_area = (*area_names_list_of_ranks)[out_edge.target_rank][target_node_areaID];
...
std::unique_ptr<AreaConnectivity::AreaConnecMap> AreaConnectivity::area_connectivity_strength_sequential(const DistributedGraph& graph,
													 unsigned int result_rank) {
	...
	MPIWrapper::gatherv<int>(area_names_char_len.data(), nbr_area_names, global_area_names_char_len.data(), rank_to_number_area_names.data(),
				 char_len_displ.data(), MPI_INT, result_rank);
	MPIWrapper::barrier();
	...
	MPIWrapper::gatherv<char>(transmit_area_names_string.data(), nbr_string_chars, global_area_names_string.data(), rank_to_string_len.data(),
				  char_displ.data(), MPI_CHAR, result_rank);
	MPIWrapper::barrier();
	...
	if (my_rank == result_rank) {
		...
		for (int r = 0; r < rank_to_area_names_char_len.size(); r++) {
			for (int l = 0; l < rank_to_area_names_char_len[r].size(); l++) {
				...
				std::string name(&global_area_names_string[displacement], rank_to_area_names_char_len[r][l]);
		...
		for (int r = 0; r < number_ranks; r++) {
			for (int n = 0; n < node_numbers[r]; n++) {
				...
				for (const OutEdge& out_edge : out_edges) {
					...
					std::uint64_t target_area_localID = graph.get_node_area_localID(out_edge.target_rank, out_edge.target_id);
					
					std::string source_area_str = rank_to_area_names[r][source_area_localID];
					std::string target_area_str = rank_to_area_names[out_edge.target_rank][target_area_localID];
