...
// Build local area connection map
    ...
    std::function<std::unique_ptr<std::vector<std::tuple<std::uint64_t,std::uint64_t,AreaConnectivityInfo>>>
        ...
        transfer_connection_sources = [&](const DistributedGraph& dg,std::uint64_t node_local_ind)
        {
            ...
            std::make_unique<std::vector<std::tuple<std::uint64_t,std::uint64_t,AreaConnectivityInfo>>>(out_edges.size());
            for(int i=0; i<out_edges.size(); i++)
            {
                AreaConnectivityInfo one_connection = {my_rank,node_areaID,out_edges[i].target_rank,-1,out_edges[i].weight};
                (*connection_sources)[i] = std::tie(out_edges[i].target_rank,out_edges[i].target_id,one_connection);
            }
...
std::unique_ptr<AreaConnectivity::AreaConnecMap> AreaConnectivity::area_connectivity_strength_sequential_helge
(
    const DistributedGraph& graph,
...
std::unique_ptr<AreaConnectivity::AreaConnecMap> AreaConnectivity::area_connectivity_strength_sequential
(
    const DistributedGraph& graph,
    unsigned int result_rank
)
