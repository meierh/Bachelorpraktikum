...
public:
	...
	//SCOREP_USER_REGION_END(region_handle_SetAnswers)
        ...
        if(my_rank==0)
        {
            ...
            std::cout<<"    >>>> node_to_node_question <<<<"<<std::endl;
...
private:
    ...
    )
    {
        ...
        if(my_rank==0)
        {
            /*
            std::cout.precision(5);
            std::cout<<"    >>>> gather_Data  "<<root<<" <<<<"<<std::endl;
            for(int i=0;i<names.size();i++)
            {
                std::cout<<"    "<<names[i]<<":\tavg:"<<global_avg_times_double[i]<<"\tmax:"<<global_max_times_double[i]<<"   milliseconds"<<std::endl;
            }
