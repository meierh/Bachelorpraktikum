...
public:
	...
	static std::unique_ptr<NodeToNodeQuestionStructure<Q_parameter, A_parameter>> node_to_node_question(
	    ...
	    const std::function<std::unique_ptr<std::vector<std::tuple<std::uint64_t, std::uint64_t, Q_parameter>>>(
	    	const DistributedGraph& dg, std::uint64_t node_local_ind)>generateAddressees,
	...
	static std::unique_ptr<std::vector<std::vector<DATA>>> gather_Data_to_one_Rank(
	    const DistributedGraph& dg, std::function<std::unique_ptr<std::vector<std::pair<DATA, int>>>(const DistributedGraph&)> getData,
