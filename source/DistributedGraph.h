...
public:
	...
	Vec3d get_node_position(int mpi_rank, int node_id) const {
		Vec3d position;
		MPIWrapper::passive_sync_rma_get<Vec3d>(&position, 1, node_id, mpi_rank, MPIWrapper::MPI_Vec3d, nodes_window);
		return position;
	}

	std::uint64_t get_node_area_localID(int mpi_rank, int node_id) const {
		std::uint64_t area_name_ind;
		MPIWrapper::passive_sync_rma_get<std::uint64_t>(&area_name_ind, 1, node_id, mpi_rank, MPI_UINT64_T, area_names_ind_window);
		return area_name_ind;
	}

	std::uint64_t get_node_signal_localID(int mpi_rank, int node_id) const {
		std::uint64_t signal_type_ind;
		MPIWrapper::passive_sync_rma_get<std::uint64_t>(&signal_type_ind, 1, node_id, mpi_rank, MPI_UINT64_T, signal_types_ind_window);
		return signal_type_ind;
	}

	std::uint64_t get_number_in_edges(int mpi_rank, int node_id) const {
		std::uint64_t number_in_edges;
		MPIWrapper::passive_sync_rma_get<std::uint64_t>(&number_in_edges, 1, node_id, mpi_rank, MPI_UINT64_T, number_in_edges_window);
		return number_in_edges;
	}

	std::uint64_t get_number_out_edges(int mpi_rank, int node_id) const {
		std::uint64_t number_out_edges;
		MPIWrapper::passive_sync_rma_get<std::uint64_t>(&number_out_edges, 1, node_id, mpi_rank, MPI_UINT64_T, number_out_edges_window);
		return number_out_edges;
	}

	InEdge get_in_edge(int mpi_rank, int node_id, int edge_id) const {
		std::uint64_t prefix_in_edge;
		MPIWrapper::passive_sync_rma_get<std::uint64_t>(&prefix_in_edge, 1, node_id, mpi_rank, MPI_UINT64_T, prefix_in_edges_window);
		InEdge edge;
		MPIWrapper::passive_sync_rma_get<InEdge>(&edge, 1, prefix_in_edge + edge_id, mpi_rank, MPIWrapper::MPI_InEdge, in_edges_window);
		return edge;
	}
	...
	const std::vector<InEdge>& get_in_edges(int mpi_rank, int node_id) const {
		if (!in_edge_cache.contains(mpi_rank, node_id)) {
			...
			MPIWrapper::passive_sync_rma_get<InEdge>(edges.data(), number_in_edges, prefix_in_edge, mpi_rank, MPIWrapper::MPI_InEdge,
								 in_edges_window);
	...
	OutEdge get_out_edge(int mpi_rank, int node_id, int edge_id) const {
		...
		MPIWrapper::passive_sync_rma_get<std::uint64_t>(&prefix_out_edge, 1, node_id, mpi_rank, MPI_UINT64_T, prefix_out_edges_window);
		OutEdge edge;
		MPIWrapper::passive_sync_rma_get<OutEdge>(&edge, 1, prefix_out_edge + edge_id, mpi_rank, MPIWrapper::MPI_OutEdge, out_edges_window);
		return edge;
	}
	...
	const std::vector<OutEdge>& get_out_edges(int mpi_rank, int node_id) const {
		if (!out_edge_cache.contains(mpi_rank, node_id)) {
			...
			MPIWrapper::passive_sync_rma_get<OutEdge>(edges.data(), number_out_edges, prefix_out_edge, mpi_rank, MPIWrapper::MPI_OutEdge,
								  out_edges_window);
