...
double Modularity::compute_modularity(DistributedGraph& graph) {
	...
	std::function<std::unique_ptr<std::vector<std::pair<std::string, int>>>(const DistributedGraph&)> get_data = [&](const DistributedGraph& dg) {
	...
	std::function<std::unique_ptr<std::vector<std::tuple<std::uint64_t, std::uint64_t, std::uint64_t>>>(const DistributedGraph& dg,
													    std::uint64_t node_localID)>
	    collect_adjacency_area_info = [&](const DistributedGraph& dg, std::uint64_t node_localID) {
		    ...
		    auto outward_node_area = std::make_unique<std::vector<std::tuple<std::uint64_t, std::uint64_t, std::uint64_t>>>(out_edges.size());

		    std::transform(out_edges.cbegin(), out_edges.cend(), outward_node_area->begin(), [&](const OutEdge& oEdge) {
			    return std::tuple<std::uint64_t, std::uint64_t, std::uint64_t>{oEdge.target_rank, oEdge.target_id, area_globalID};
		    });

		    return outward_node_area;
	    };

	std::function<std::uint8_t(const DistributedGraph& dg, std::uint64_t node_localID, std::uint64_t area_globalID)> test_for_adjacent_equal_area =
	...
	adjacency_results = CommunicationPatterns::node_to_node_question<std::uint64_t, std::uint8_t>(graph, MPI_UINT64_T, collect_adjacency_area_info,
	...
	for (int area_globalID = 0; area_globalID < area_names_list.size(); area_globalID++) {
		...
		std::function<std::unique_ptr<std::vector<std::pair<NodeModularityInfo, int>>>(const DistributedGraph&)> get_modularity_data =
		    [&](const DistributedGraph& dg) {
			    
			    auto data = std::make_unique<std::vector<std::pair<NodeModularityInfo, int>>>(area_local_size);

			    std::transform(area_globalID_to_node[area_globalID].cbegin(), area_globalID_to_node[area_globalID].cend(), data->begin(),
					   [](NodeModularityInfo modul_info) { return std::pair<NodeModularityInfo, int>(modul_info, 1); });
		...
		if (my_rank == calculation_rank) {
			...
			std::for_each(local_area_globalID_to_nodeID->begin(), local_area_globalID_to_nodeID->end(),
				      [&](std::vector<NodeModularityInfo> modul_info_vec) {
					      ...
					      local_area_globalID_to_node.back().insert(local_area_globalID_to_node.back().end(), modul_info_vec.begin(),
											modul_info_vec.end());
				      });
		}
	}
		...
		for (int i = 0; i < nodes_of_area.size(); i++) {
			for (int j = 0; j < nodes_of_area.size(); j++) {
				...
				local_in_out_degree_node_sum -=
				    static_cast<double>((nodes_of_area[i].node_in_degree * nodes_of_area[j].node_out_degree)) / global_m;
			}
		}
	}
